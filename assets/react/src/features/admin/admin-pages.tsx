...
export function AdminUsersPage() {
  ...
  const querySudoError = users.error
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        eyebrow="Administration"
        title="Users"
        description="Accounts, access, and linked players."
      />
...
export function AdminSettingsPage() {
  ...
  if (isSudoRequired(settings.error)) {
    return <SudoPrompt error={settings.error} onSuccess={() => void settings.refetch()} />
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        description="Repair imported card references against the local Scryfall catalog."
      />
      <CatalogBackfill />
    </div>
  )
...
export function AdminDiscordPage() {
  ...
  if (isSudoRequired(games.error)) {
    return (
      <SudoPrompt
        error={games.error}
        onSuccess={() => void queryClient.invalidateQueries({ queryKey: ["admin", "discord"] })}
      />
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        eyebrow="Administration"
        title="Discord reports"
        description="Resolve or discard incomplete SpellBot game reports."
      />
      <PendingDiscordGames games={games.data} error={games.error} />
    </div>
  )
}
...
function UserCard({ user, players }: { user: User; players: PlayerSummary[] }) {
  const queryClient = useQueryClient()
 ...
 */
function LinkedPlayer({ user, players }: { user: User; players: PlayerSummary[] }) {
  const queryClient = useQueryClient()
  const current = players.find((player) => player.user_id === user.id)
  const [pending, setPending] = useState<PlayerSummary | null>(null)
  const link = useMutation({
    mutationFn: (player: PlayerSummary) => linkUserPlayer(user.id, player.id),
    onSuccess: () => {
