...
function harness(participants: TableParticipant[] = [owner]) {
  ...
  let listener: { message?: (peerId: string, message: DataMessage) => void; left?: (peerId: string) => void }
...
it("reassembles out-of-order chunks, tolerates exact duplicates, and validates SHA-256", async () => {
  ...
  transport.message({ type: "super_ai_frame_start", requestId: "request", width: 2, height: 3, bytes: 6, chunks: 2, digest, private: false })
  ...
  await waitFor(() => expect(transport.onFrame).toHaveBeenCalledWith({ bytes: data, width: 2, height: 3 }))

  const bad = harness()
  act(() => expect(bad.hook.result.current.request("owner")).toBe(true))
  bad.message({ type: "super_ai_frame_start", requestId: "request", width: 2, height: 3, bytes: 3, chunks: 1, digest: "0".repeat(64), private: false })
...
it("rejects conflicting duplicates, oversized reassembly, and malformed base64", () => {
  const transport = harness()
  act(() => transport.hook.result.current.request("owner"))
  const start = { type: "super_ai_frame_start" as const, requestId: "request", width: 2, height: 3, bytes: 3, chunks: 1, digest: "0".repeat(64), private: false }
...
it("stops accepted transfers when visibility is revoked and rate-limits requests", () => {
  vi.useFakeTimers()
  const transport = harness()
