defmodule TheGatheringWeb.WebcamTableTurns do
  ...
  def next_player(seats, active_id) do
  ...
  def pass(turns, seats, elapsed) do
    next = next_player(seats, turns.active_player_id)
  ...
  def reconcile(turns, seats, elapsed) do
    active? = Enum.any?(seats, &(&1.player_id == turns.active_player_id and eligible?(&1)))

    if active? or (is_nil(turns.active_player_id) and is_nil(next_player(seats, nil))),
      do: turns,
      else: pass(turns, seats, elapsed)
  end
